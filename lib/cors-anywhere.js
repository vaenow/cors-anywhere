...
var { randomPickCorsHost } = require('./cors-ext')
// const { ungzip } = require('node-gzip');
const OrderCacheV2 = require('./OrderCacheV2')
  ...
  try {
    proxy.on('proxyRes', function (proxyRes, req, res) {
      ...
      proxyRes.on('end', async function () {
        ...
        // console.log("res from proxied server:", decompressed.toString());

        OrderCacheV2.set(getCacheKey(req), {req, res, data: buffer, rspHeaders: proxyRes.headers, rspStatusCode: proxyRes.statusCode}, {ttl: +process.env.REQ_CACHE_TTL || 60e3})
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    // 使用 cache req
    const cache = OrderCacheV2.get(getCacheKey(req))

    // 重跳转 balancer
    var corsHost = randomPickCorsHost()
    corsHost = (corsHost.length && !corsHost.match(/^\//) ? '/' :  '') + corsHost
    console.log(`${corsHost} => ${req.url} ${cache ? '[cached]' : ''}`)
    req.url = corsHost + req.url

    if (cache) {
      res.writeHead(cache.rspStatusCode,  JSON.parse(JSON.stringify(cache.rspHeaders)));
      res.end(cache.data)
      return
    }
