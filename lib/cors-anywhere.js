...
function proxyRequest(req, res, proxy) {
  ...
  try {
    if (cachedRes[cacheKey]) {
      res.writeHead(cachedRes[cacheKey].rspStatusCode,  JSON.parse(JSON.stringify(cachedRes[cacheKey].rspHeaders)));
      res.end(cachedRes[cacheKey].data)
      return
    }
    ...
    proxy.on('proxyRes', function (proxyRes, req, res) {
      ...
      proxyRes.on('end', async function () {
        ...
        // cachedRes[cacheKey] = decompressed.toString()
        cachedRes[cacheKey] = {req, res, data: buffer, rspHeaders: proxyRes.headers, rspStatusCode: proxyRes.statusCode}
